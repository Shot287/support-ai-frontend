// src/features/study/lisp-dictionary.tsx
"use client";
...
// v1 既存データ用（将来のために残しておくが、現状は未使用でもOK）
type EntryV1 = {
  id: ID;
  term: string;
  meaning: string;
...
/* ========= 定数 / ユーティリティ ========= */
// ローカル保存用キー（Lisp用）
const LOCAL_KEY_V2 = "lisp_dictionary_v2";
const LOCAL_KEY_V1 = "lisp_dictionary_v1";

// user_docs 用の doc_key（Lisp用）
const DOC_KEY = "study_lisp_dictionary_v1";
...
/* ========= 本体 ========= */
export default function LispDictionary() {
    ...
    const unsubscribe = registerManualSync({
      ...
      pull: async () => {
        try {
          ...
          console.warn("[lisp-dictionary] manual PULL failed:", e);
        }
      },
      // ☁ アップロード（ローカル→クラウド）
      push: async () => {
        try {
          ...
          console.warn("[lisp-dictionary] manual PUSH failed:", e);
        }
      },
      // ⚠ RESET: since 未使用なので特別な処理はなし
      reset: async () => {
        /* no-op */
      },
    });
  ...
  const exportJson = () => {
    ...
    a.href = url;
    a.download = `lisp_dictionary_export_${Date.now()}.json`;
    a.click();
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="rounded-2xl border p-4 shadow-sm">
        <h2 className="font-semibold mb-3">Lisp用語を追加</h2>
        <p className="mb-2 text-xs text-gray-500">
          例: term = <code>cons</code> / <code>car</code> /{" "}
          <code>S式</code> など。意味は日本語でOKです。
        </p>
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="意味（例：新しいペア（リスト）を作る基本演算）"
          ...
          <input
            ...
            placeholder="読み（任意・ひらがな。こんす / かー / えすしき など）"
            className="rounded-xl border px-3 py-3"
            aria-label="読み"
          />
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) =>
                setSortKey(
                  e.target.value as "createdAt" | "updatedAt" | "term"
                )
              }
              className="rounded-xl border px-2 py-2 text-sm"
            >
              <option value="term">用語（50音 / アルファベット順）</option>
