...
export default function Dictionary() {
  ...
  const filtered = useMemo(() => {
    ...
    const hit = nq
      ? list.filter(
          (e) =>
            normalize(e.term).includes(nq) || normalize(e.meaning).includes(nq)
        )
      : list;
    // ソート
    hit.sort((a, b) => {
  ...
  const add = () => {
    ...
    setStore((s) => ({ ...s, entries: [newEntry, ...s.entries] }));
    setTerm("");
    setMeaning("");
  ...
  const commitEdit = () => {
    ...
    setStore((s) => ({
      ...
      entries: s.entries.map((x) =>
        x.id === editingId ? { ...x, term: t, meaning: m, updatedAt: nowMs } : x
      ),
    }));
    setEditingId(null);
  };
  ...
  const exportJson = () => {
    ...
    a.href = url;
    a.download = `dictionary_export_${ts}.json`;
    a.click();
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="rounded-2xl border p-4 shadow-sm">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="意味（例：周期関数を三角関数の和で表す展開）"
        ...
        <p className="text-xs text-gray-500 mt-2">※ 後から編集・削除できます。</p>
      </section>

      {/* 検索・並び替え・インポート/エクスポート */}
      <section className="rounded-2xl border p-4 shadow-sm">
        <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-center">
          <input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="検索（用語・意味を両方検索）"
            className="rounded-xl border px-3 py-3"
            aria-label="検索"
          />
