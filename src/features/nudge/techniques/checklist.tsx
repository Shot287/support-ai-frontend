...
export default function Checklist() {
  ...
  // ★修正ポイント：終了時に次のアクションへページ送り＋次の行動までの先延ばしを自動開始
  const endActionInternal = (actionId: ID) => {
    ...
    setStore((prev) => {
      ...
      // ランのログ確定
      const runId = prev.current.runId;
      ...
      // 次の表示インデックスを計算（現在セットの総数から算出）
      const setForCalc = prev.sets.find((s) => s.id === prev.current!.setId);
      const total = setForCalc ? setForCalc.actions.length : 0;
      const nextIndex = Math.min(
        (prev.current!.index ?? 0) + 1,
        Math.max(0, total - 1)
      );
