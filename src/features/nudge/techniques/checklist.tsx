...
export default function Checklist() {
  ...
  // 行動終了：最後ならラン終了／続きがあれば先延ばし開始＋ページ送り
  ...
  const toggleDeviation = () => {
    setStore((prev) => {
      ...
      // 開いていなければ開始（現在実行中の行動に紐づけ）
      const runningActionId = cur.running?.actionId;
      if (!runningActionId) return prev; // 型・実行の両面で安全に
      return {
        ...prev,
        current: { ...cur, deviating: { actionId: runningActionId, startAt: now() } },
      };
    });
  };
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500">
              「開始」は1番目の行動を始めるまでを先延ばしとして計測。「終了」は実行中/先延ばし/逸脱を確定してこのランを閉じます。
            </p>
          </div>
