...
export default function Checklist() {
  ...
  const endActionInternal = (actionId: ID) => {
    ...
    setStore((prev) => {
      ...
      const runs = prev.runs.map((r) => {
        ...
        // 行動ログをクローズ
        const i = next.actions.findIndex((l) => l.actionId === actionId && !l.endAt);
        if (i >= 0) {
          const log = next.actions[i];
          next.actions[i] = { ...log, endAt: endedAt, durationMs: endedAt - log.startAt };
        }
      ...
      return {
        ...
        current: {
          ...
          deviating: cur.deviating && cur.deviating.actionId === actionId ? undefined : cur.deviating,
        },
      };
    });
  };
  ...
  const toggleDeviation = () => {
    setStore((prev) => {
      ...
      // 逸脱開始：ここで runningId を安全に取り出す（TSエラー解消）
      const runningId = cur.running?.actionId;
      if (!runningId) return prev;

      return {
        ...prev,
        current: { ...cur, deviating: { actionId: runningId, startAt: now() } },
      };
    });
  };
