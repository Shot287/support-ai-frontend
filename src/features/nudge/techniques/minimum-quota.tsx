...
export default function MinimumQuota() {
  ...
  /** ★修正：前日のノルマの内容をコピーして、新しいID（実体）としてこの日に作成する */
  const copyAllFromPreviousDay = () => {
    setStore((s) => {
      ...
      // --- 修正箇所 START ---
      // 前日のIDリストをループし、内容をコピーした「新しいIDと新しいオブジェクト」を作る
      const newQuotaIds: string[] = [];
      const newQuotasMap: Record<string, Quota> = {};

      for (const oldId of prevDay.quotaIds) {
        const originalQuota = s.quotas[oldId];
        // データ不整合などで無ければスキップ
        if (!originalQuota) continue;

        const newId = uid(); // 新しいIDを発行
        newQuotaIds.push(newId);
      ...
      return {
        ...s,
        // 新しく作ったQuotaオブジェクトたちを store.quotas にマージ
        quotas: { ...s.quotas, ...newQuotasMap },
        days: {
          ...s.days,
          [targetDate]: {
            quotaIds: newQuotaIds, // 新しいIDリストを使用
            checks: {}, // 揃えた日は未チェックにする
          },
        },
      };
      // --- 修正箇所 END ---
