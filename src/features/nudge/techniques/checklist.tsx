...
export default function Checklist() {
  ...
  // ✅ 追加：チェックリスト全体の開始（1番目の行動開始までを「先延ばし」として計測）
  const startChecklist = () => {
    if (!currentSet || actionsSorted.length === 0) {
      alert("先に行動を追加してください。");
      return;
    }
    // すでに先延ばし or 実行中なら何もしない
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-2xl border p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold">チェックリストの開始</h3>
            <p className="text-xs text-gray-500">
              押すと「1番目の行動を開始するまでの時間」を先延ばしとして計測します。
            </p>
          </div>
          <button
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              {running && running.actionId === action.id && (
                <span className="text-sm text-gray-700">
                  進行中：{fmtDuration(now() - running.startAt)}
                </span>
              )}
              {!running && procrastinating && procrastinating.fromActionId !== null && (
                <span className="text-sm text-red-600">
                  先延ばし中：{fmtDuration(now() - procrastinating.startAt)}
                </span>
              )}
            </div>
          </>
