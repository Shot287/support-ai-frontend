...
function parseApiError(e: unknown): { status?: number; detailText?: string } {
  ...
  // 1) ステータス番号（最初に見つかった 3 桁）を緩く拾う
  const num = msg.match(/\b(\d{3})\b/);
  const status = num ? Number(num[1]) : undefined;

  // 2) 末尾の JSON を抽出（最も右の { … } を拾う）
  let detailText: string | undefined;
  const jsonTail = msg.match(/\{[\s\S]*\}\s*$/);
  if (jsonTail) {
    try {
      const j = JSON.parse(jsonTail[0]) as unknown;
      if (j && typeof j === "object" && "detail" in (j as Record<string, unknown>)) {
        const d = (j as Record<string, unknown>).detail as unknown;
