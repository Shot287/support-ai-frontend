  ...
  const view = useMemo(() => {
    return runs.map((run) => {
      ...
      for (const a of actionsSorted) {
        ...
        // この行動中の逸脱合計/回数を集計
        const related = deviations.filter(
          (d) =>
            d.actionId === a.actionId &&
            d.startAt >= a.startAt &&
            (a.endAt ? d.startAt <= a.endAt : true)
        );
              ...
              <table className="min-w-[880px] w-full text-sm">
                ...
                <tbody>
                  ...
                  <tr className="border-t font-medium">
                    ...
                    <td className="py-2 pr-3 tabular-nums" colSpan={2}>
                      {fmtDur(v.sumDev)}（逸脱合計） / {fmtDur(v.sumAction)}（行動合計）
                    </td>
                  </tr>
                </tbody>
              </table>
