...
export default function Tournament() {
  ...
  const [autoAdvancing, setAutoAdvancing] = useState(false); // 自動進行中フラグ
  ...
  // ① 不戦勝の自動消化：b が null の試合は自動で勝者確定して次へ
  useEffect(() => {
    if (step !== "play") return;
  ...
  // ② ラウンド終了の自動遷移：最後の試合が終わったら次ラウンド or 結果へ
  useEffect(() => {
    if (step !== "play") return;
    ...
    setCursor(0);
    setNextIds([]);
  }, [step, cursor, pairs.length, nextIds, tasks, history]);
