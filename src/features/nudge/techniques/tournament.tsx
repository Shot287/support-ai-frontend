...
function uid() {
  // ブラウザで使えるときは衝突がほぼない UUID を採用
  if (typeof crypto !== "undefined" && "randomUUID" in crypto) {
    // @ts-ignore 型環境によっては randomUUID の型定義が無いケースがあるため保険
    return crypto.randomUUID();
  }
  // フォールバック（衝突確率を下げるために時刻＋乱数を混ぜる）
...
export default function Tournament() {
  ...
  const addTask = () => {
    const raw = titleInput;
    const title = raw.trim();
    if (!title) return;
    setTasks(prev => {
      const isDup = prev.length > 0 && prev[prev.length - 1].title === title;
