...
/** JST 0時のUnix ms（UTC基準） */
function jstDayStartMs(yyyyMmDd: string): number {
  // 明示的に +09:00 を付与してUTCに変換
  return Date.parse(`${yyyyMmDd}T00:00:00+09:00`);
}

/** 表示用：JSTの日時（YYYY/MM/DD HH:mm:ss） */
function formatJST(ts: number | string | Date): string {
  try {
...
/** input[type=datetime-local] 用の値（JST） YYYY-MM-DDTHH:mm */
...
/** inputからJSTとして解釈してutc msに */
function parseInputAsJST(input: string): number {
  // "YYYY-MM-DDTHH:mm" → +09:00 を付けてUTCへ
...
export default function WorkLog() {
  ...
  return (
    <div className="rounded-2xl border p-4 sm:p-6 shadow-sm grid gap-6">
      {/* 上段：グループ＆カード作成（モバイル1カラム → smで2カラム） */}
...
// 旧: UTC表示 → JST表示に差し替え（使っていないが互換保持）
// function fmtTime(d: Date) { ... }  // 互換のため残すなら fmtTimeJST を使用
