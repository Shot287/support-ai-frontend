...
/** エラーメッセージからHTTPステータス/詳細を推定（api.tsのエラーフォーマット対応） */
function parseApiError(e: unknown): { status?: number; detailText?: string } {
  const msg = e instanceof Error ? e.message : String(e);
  // 例: "API 409 Conflict: {"detail":"already running"}"
  const m = msg.match(/API\s+(\d{3})[^\:]*:\s*(.*)$/);
  const status = m ? Number(m[1]) : undefined;
  const tail = m ? m[2] : undefined;
  let detailText: string | undefined;
  if (tail) {
    try {
      const j = JSON.parse(tail) as unknown;
      if (typeof j === "object" && j && "detail" in (j as Record<string, unknown>)) {
