...
export default function Checklist() {
  ...
  const go = (i: number) =>
    setStore((s) => ({
      ...s,
      current: s.current
        ? { ...s.current, index: Math.max(0, Math.min(i, maxIndex)) }
        : s.current,
    }));
  ...
  const endChecklist = () => {
    ...
    if (deviating) {
      ...
      setStore((s) => ({
        ...s,
        current: { ...s.current!, deviating: undefined },
        runs: s.runs.map((r) =>
          r.id !== s.current!.runId
            ...
            : {
                ...
                deviations: [
                  ...r.deviations,
                  {
                    actionId: deviating.actionId,
                    startAt: deviating.startAt,
                    endAt: endedAt,
                    durationMs: endedAt - deviating.startAt,
                  },
                ],
              }
        ),
    ...
    // 実行中の行動 → 終了
    if (running) endActionInternal(running.actionId);
    // 先延ばし中 → 打ち切り（終了時刻で確定）
    if (procrastinating) {
      const endedAt = now();
      setStore((s) => ({
          ...
          r.id !== s.current!.runId
            ...
            : {
                ...
                procrastinations: [
                  ...r.procrastinations,
                  {
                    fromActionId: procrastinating.fromActionId,
                    startAt: procrastinating.startAt,
                    endAt: endedAt,
                    durationMs: endedAt - procrastinating.startAt,
                  },
                ],
                endedAt, // ラン終了時刻
              }
      ...
      // 先延ばしでない場合もランに終了時刻だけ付与
      const endedAt = now();
      setStore((s) => ({
        ...s,
        runs: s.runs.map((r) =>
          r.id === s.current!.runId ? { ...r, endedAt } : r
        ),
      }));
    }
    // 現在状態の実行フラグを全解除
    setStore((s) => ({
      ...s,
      current: s.current ? { ...s.current, running: undefined, procrastinating: undefined, deviating: undefined } : s.current,
    }));
  };
  ...
  const endActionInternal = (actionId: ID) => {
    ...
    // 行動中の逸脱が開いていれば、ここで自動終了
    if (deviating && deviating.actionId === actionId) {
      setStore((s) => ({
        ...s,
        current: { ...s.current!, deviating: undefined },
        runs: s.runs.map((r) =>
  ...
  const toggleDeviation = () => {
    if (!running) {
      alert("逸脱は行動中のみ記録できます。先に「開始」を押してください。");
      return;
    }
    // 逸脱終了
    if (deviating) {
    ...
    setStore((s) => ({
      ...s,
      current: { ...s.current!, deviating: { actionId: running.actionId, startAt: now() } },
    }));
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500">
              「開始」で1番目の行動を始めるまでを先延ばしとして計測します。途中で止めたい場合は「終了」。
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              ...
              title="実行中の行動/先延ばし/逸脱をクローズしてこのランを終了します"
            >
              チェックリスト終了
            </button>
          </div>
        ...
        {action ? (
          <>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              {!running || running.actionId !== action.id ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className={`rounded-xl px-5 py-3 ${deviating ? "bg-red-600 text-white" : "border hover:bg-gray-50"}`}
                    title="行動中にSNSなどへ逸れた時間を記録します"
                  >
                    {deviating ? "逸脱終了" : "逸脱開始"}
                  </button>
                </>
              ...
              {deviating && running && running.actionId === deviating.actionId && (
