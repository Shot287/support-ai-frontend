...
// Server types
interface ServerGroup { id: string; name: string; color?: string; created_at?: string; user_id?: string }
interface ServerCard  { id: string; group_id: string; name: string; color?: string; created_at?: string; user_id?: string }
...
async function ensureCardOnServer(card: Card, parentGroup: Group): Promise<string> {
  ...
  await apiPost(`/nudge/work-log/cards`, { group_id: groupServerId, name: card.name, color: card.color });
...
export default function ManageWorkLog() {
  ...
  const renameGroup = (id: ID) => {
    const g = groupMap[id]; if (!g) return; setRenamingGroupId(id); setTmpName(g.name);
  };
  const commitRenameGroup = () => {
    if (!renamingGroupId) return; const name = tmpName.trim(); if (!name) return;
    setStore(s => ({ ...s, groups: s.groups.map(g => g.id === renamingGroupId ? { ...g, name, color: pickColor(name) } : g) }));
    setRenamingGroupId(null); setTmpName("");
  };
  const deleteGroup = (id: ID) => {
    const g = groupMap[id]; if (!g) return;
    const count = store.cards.filter(c => c.groupId === id).length;
    if (!confirm(`グループ「${g.name}」を削除します。所属カード ${count} 件も削除されます。よろしいですか？`)) return;
    setStore(s => ({
      ...s,
  ...
  const addCard = async () => {
    ...
    const name = cardName.trim(); if (!name) return;
    const local: Card = { id: uid(), groupId: selectedGroupId as ID, name, color: pickColor(name), createdAt: Date.now() };
    setStore(s => ({ ...s, cards: [...s.cards, local] }));
    setCardName("");

    try {
      const parent = groupMap[selectedGroupId as string]; if (!parent) throw new Error("group not found");
  ...
  const renameCard = (id: ID) => {
    const c = store.cards.find(x => x.id === id); if (!c) return; setRenamingCardId(id); setTmpName(c.name);
  };
  const commitRenameCard = () => {
    if (!renamingCardId) return; const name = tmpName.trim(); if (!name) return;
    setStore(s => ({ ...s, cards: s.cards.map(c => c.id === renamingCardId ? { ...c, name, color: pickColor(name) } : c) }));
    setRenamingCardId(null); setTmpName("");
  };
    ...
    try {
      ...
      for (const g of store.groups) {
        if (!g.serverId) {
          ...
          setStore(s => ({ ...s, groups: s.groups.map(x => x.id === g.id ? { ...x, serverId: sid } : x) }));
        }
      }
      // cards
      for (const c of store.cards) {
        if (!c.serverId) {
          const parent = groupMap[c.groupId]; if (!parent) continue;
          const sid = await ensureCardOnServer(c, parent);
          setStore(s => ({ ...s, cards: s.cards.map(x => x.id === c.id ? { ...x, serverId: sid } : x) }));
        }
      }
  ...
  /* ---------- 描画 ---------- */
  return (
    <main className="max-w-5xl app-width-guard">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h1 className="text-2xl font-bold">カードグループ／カード管理</h1>
        <Link href="/nudge/work-log" className="rounded-xl border px-4 py-2 hover:bg-gray-50">← 作業記録へ戻る</Link>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <section className="rounded-2xl border p-4 shadow-sm">
          ...
          <ul className="mt-4 space-y-2">
            {store.groups
              ...
              .map(g => {
                const count = store.cards.filter(c => c.groupId === g.id).length;
                  ...
                  <li key={g.id} className="rounded-xl border px-3 py-2">
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {!isEditing ? (
                            <>
                              ...
                              <span className="text-xs text-gray-500">({count}) {g.serverId ? "" : "未同期"}</span>
                            </>
                          ) : (
                            <input
                              autoFocus
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button onClick={()=>setSelectedGroupId(g.id)} className={`rounded-lg border px-2 py-1 text-sm ${selectedGroupId===g.id?"bg-gray-100":""}`}>選択</button>
                        {!isEditing ? (
                          <>
                            <button onClick={()=>renameGroup(g.id)} className="rounded-lg border px-2 py-1 text-sm">名称変更</button>
                            <button onClick={()=>deleteGroup(g.id)} className="rounded-lg border px-2 py-1 text-sm">削除</button>
                          </>
                        ) : (
                          <>
                            <button onClick={commitRenameGroup} className="rounded-lg border px-2 py-1 text-sm">保存</button>
                            <button onClick={()=>{setRenamingGroupId(null); setTmpName("");}} className="rounded-lg border px-2 py-1 text-sm">取消</button>
                          </>
                        )}
                      </div>
                    </div>
        ...
        <section className="rounded-2xl border p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-sm text-gray-600">{selectedGroupId ? `対象: ${groupMap[selectedGroupId]?.name ?? "?"}` : "対象: すべて"}</div>
          </div>
                  ...
                  <li key={c.id} className="rounded-xl border px-3 py-2">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0 flex items-center gap-2">
                        ...
                        {!isEditing ? (
                          <>
                            ...
                            <span className="text-xs text-gray-500">（{groupMap[c.groupId]?.name ?? "—"}）{c.serverId ? "" : " 未同期"}</span>
                          </>
                        ) : (
                          <input
                            autoFocus
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {!isEditing ? (
                          <>
                            <button onClick={()=>renameCard(c.id)} className="rounded-lg border px-2 py-1 text-sm">名称変更</button>
                            <button onClick={()=>deleteCard(c.id)} className="rounded-lg border px-2 py-1 text-sm">削除</button>
                          </>
                        ) : (
                          <>
                            <button onClick={commitRenameCard} className="rounded-lg border px-2 py-1 text-sm">保存</button>
                            <button onClick={()=>{setRenamingCardId(null); setTmpName("");}} className="rounded-lg border px-2 py-1 text-sm">取消</button>
                          </>
                        )}
                      </div>
                    </div>
      ...
      <section className="rounded-2xl border p-4 shadow-sm mt-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={resyncMissing} className="rounded-xl border px-4 py-2 hover:bg-gray-50">未同期を一括同期</button>
          <button
            onClick={() => {
              if (!confirm("ローカルのグループ・カードを全消去します。よろしいですか？（サーバには影響しません）")) return;
